...
states.subset.ME <- filter(states, ID %in% "maine")
...
```{r}
secchi.summary.ME <- LAGOScombined %>%
  filter(state == "ME") %>% 
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
...
```{r}
secchi.spatial.ME <- st_as_sf(secchi.summary.ME, coords = c("long", "lat"), crs = 4326)

ggplot() +
  geom_sf(data = states.subset.ME, fill = "white") +
...
```{r}
secchi.ME.Max <- LAGOScombined %>%
  filter(state == "ME") %>% 
  group_by(lagoslakeid) %>%
  summarise(secchi.max = max(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
            lat = mean(nhd_lat), 
            long = mean(nhd_long)) %>%
...
secchi.spatial.ME.Max <- st_as_sf(secchi.ME.Max, coords = c("long", "lat"), crs = 4326)

ggplot() +
  geom_sf(data = states.subset.ME, fill = "white") +
  geom_sf(data = secchi.spatial.ME.Max, aes(color = secchi.max), 
          alpha = 0.5, size = "Max.Depth (m)") +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
