...
# Y is for 4 digits, small y is for 2 digits 
class(NTLdata$sampledate)
...
dim(NTLdata)  
# Since we alredy told where is the file, we can just specify
...
ggplot(subset(Pauldata, year4 == 1993), 
  ...
  scale_y_reverse() +
  scale_color_viridis_c() +
  labs(x = expression("DO"(mg/L)), y = "Depth (m)", color = "DOY") #lower concentration at the bottom, on the surface higher 

#The bulge is called: deep chlorocal maximum
```

Why does mixing occur in the spring and fall? What are the mechanisms that make this possible?
<add notes here>

Desnity gets smaller as the appelum become smaller 
...
Pauldata_thermo <- Pauldata %>%
 ...
 ## >1 degree over less than 1m of depth
...
<add notes here>
##Thermocline deepening as the years go on. More variability early in the year 
