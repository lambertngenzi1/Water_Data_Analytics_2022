...
```{r}
NeuseParams <- whatNWISdata(siteNumbers = "02089500")  # can just google USGS parameter 

NeuseWQ <- readWQPqw(siteNumbers = "USGS-02089500", # Neuse River at Kinston, NC  # Have to add uSGS before site number
...
NeuseFlow <- readNWISdv(siteNumbers = "02089500",      #Use Read for flow 
...
NeuseWQ_processed <- NeuseWQ %>%
  ...
  pivot_wider(names_from = "Variable", values_from = "ResultMeasureValue") #When you have space, things to get trickier. Better use no space in

NeuseFlow_processed <- NeuseFlow %>%
  select(ActivityStartDate = Date, Flow_cfs = X_00060_00003) %>% #How you can rename without making a new dateset
...
What is the relationship between flow and the water quality indicators? A lot of variabilities in the data
...
ggplot(NeuseFlow_summary, aes(x = DOY)) +
  ...
  scale_y_log10() +
  labs(x = "Day of Year", y = "Flow (cfs)")  # Wet and dry season graph 
