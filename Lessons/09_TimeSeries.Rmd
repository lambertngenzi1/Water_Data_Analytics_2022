...
ggplot(Neuse_Components) +
  ...
  labs(x = "", y = "Discharge (cfs)")
#LN: The trend is always positive 
...
ggplot(Neuse_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  labs(x = "", y = "Discharge (cfs)")
#LN: With seasonal goes positive and negative
#Interpret with cautious and proceed with cautious 
...
Common interpolation methods:# Filling  

* **Piecewise constant**: <fill in notes here> #LN Nearest neighbor (takes the last date and define across the gap)
* **Linear**: <fill in notes here> # Connect the dot straight (common to use a variety)
* **Spline**: <fill in notes here> # Uses quadratic formula 
...
NeuseCond_processed <- NeuseCond %>%
  ...
  mutate(Year = year(Date),
         Month = month(Date),
         Date_monthrounded = floor_date(Date, "month")) %>%  # floor_date just rounded it down 
  arrange(Date)
...
NeuseCond_monthly_Dec <- stl(NeuseCond_monthly_ts, s.window = "periodic")

plot(NeuseCond_monthly_Dec)
```
