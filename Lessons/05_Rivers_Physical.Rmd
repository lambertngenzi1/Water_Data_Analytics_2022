...
install.packages("EcoHydRology") #use a lot of packages used in tidyverse 
#do the tidyverse first (the order may fact on how the other works)
...
# very responsive to seasonal rain event 
```
...
# If something is a factor, need to make it a character first 

# plot baseflow and total flow
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
  geom_line(color = "gray") +
  geom_line(aes(y = Baseflow_lfstat)) +
  scale_y_log10() + # Using log scale is nice when comparing big numbers
  labs(x = "Year", y = "Discharge (cfs)")
# Baseflow on perennial river depends on groundwater 
...
EnoDischarge_basesep_EcoHydro <- BaseflowSeparation(EnoDischarge$Discharge) # baseflowSeparation fct calculates stormflow and baseflow at the same time 
...
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  geom_point(alpha = 0.3, size = 1) + #alpha 1 not transparent 
  geom_abline(slope = 1, intercept = 0, color = "darkred")

# How would we test that statistically? [add code here]
summary(lm(data = EnoDischarge, bt ~ Baseflow_lfstat)) # to find Y and rsquare
# Can also use a T-test to find the P-value 
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>%
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # what is this factor? Converting from cfs to ac*ft per year 
 ...
 mutate_if(is.numeric, round, 2) # notes here if you are doing any calculation, # the mutate if is to limit to 2 decimals

#Watch out for first and last year


ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2) #lty is dotted for visualization purpose
...
# BC of pumping from Ogalala aquifer, Went from pernnial and to non perennial 
...
```{r}
CoolidgeRiver <- readNWISdv(siteNumbers = "07137500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "1960-10-01", 
                     endDate = "2005-09-30")

names(CoolidgeRiver)[4:5] <- c("Discharge", "Approval.Code")

attr(CoolidgeRiver, "variableInfo")
attr(CoolidgeRiver, "siteInfo")


CoolidgeRiver_ba <- CoolidgeRiver %>%
  mutate(Baseflow_lfstat = baseflow(Discharge), 
         Stormflow_lfstat = Discharge - Baseflow_lfstat,
         Year = year(Date), 
         WaterYear = water_year(Date))

ggplot(CoolidgeRiver_ba, aes(x = Date, y = Discharge)) +
  geom_line(color = "gray") +
  geom_line(aes(y = Baseflow_lfstat)) +
  #scale_y_log10() + # Using log scale is nice when comparing big numbers
  labs(x = "Year", y = "Discharge (cfs)")
