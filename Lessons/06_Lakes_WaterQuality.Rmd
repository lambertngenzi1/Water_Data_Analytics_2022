...
options(scipen = 4) ## Will tell how many digits to get (4-10)

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE) ## A link datebase online 
...
LAGOSdata <- lagosne_load()  ##Loaded as a list not as dataframe, need to pull them out to see them 
                    #individually
names(LAGOSdata)
...
```{r}
# Join data frames
LAGOSlocations <- left_join(LAGOSlocus, LAGOSstate, by = "state_zoneid") #Don't always need to specify 
# when all the columns are the same With different columns (by = c("colname1, ...))
...
**0-40**: Oligotrophic #few (low primary productivity)
**40-50**: Mesotrophic
**50-70**: Eutrophic
...
LAGOStrophic <- LAGOSnutrient %>%
  ...
  drop_na(chla, tp, secchi, state)  ##drop_na will drop any na in any columns
...
# Higher concentration in the summer and total range is bigger 
# More measurement in the summer and less in the winter 
...
LAGOStrophic <- LAGOStrophic %>%
  ...
  mutate(TSI.chl = round(10*(6 - (2.04 - 0.68*log(chla)/log(2)))),
         ...
         trophic.class = case_when(TSI.chl < 40 ~ "Oligotrophic",   #Case_when if 
...
ggplot(LAGOStrophic, aes(x = state, y = TSI.chl)) +
  ...
  #geom_rect() color inside of the violin box 
# Violin plot shows that relative density of the point, tells you about concentration and distribution 
ggplot(LAGOStrophic, aes(x = state, fill = trophic.class)) +
  geom_bar(stat = "count", position = position_dodge(preserve = "single")) + #dodge gets them side by side not stacked, Preserve single will just give each state just o fill the space
  theme(legend.position = "top") +
