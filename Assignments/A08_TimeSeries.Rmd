...
ggplot(Clear_Creek, aes(Date, Discharge)) +
  geom_line(alpha = 0.5)

Clear_Creek_MQ <- Clear_Creek %>% 
  mutate(Year = year(Date),
         Month = month(Date)) %>% 
  group_by(Year, Month) %>% 
  summarise(Discharge = sum(Discharge)*1.98)
...
```{r}
Clear_Creek_MQ_ts <- ts(Clear_Creek_MQ$Discharge, frequency = 12
                      , start = c(1975, 1, 1), end = c(2021, 12, 1))

Clear_Creek_MQ_dec <- stl(Clear_Creek_MQ_ts, s.window = "periodic")

plot(Clear_Creek_MQ_dec)
```

8. How do the seasonal and trend components of the decomposition compare to the Neuse River discharge dataset? 

> Seasonal: seasonal trend is between -10000 and 30000 and with season 
...
```{r}
Clear_Creek_SMK <- smk.test(Clear_Creek_MQ_ts)
...
auto.arima(Clear_Creek_MQ_ts, trace = TRUE)

Best_Fit_Model <- arima(Clear_Creek_MQ_ts, c(1,0,0), seasonal = list(order = c(1,1,0), period = 12))

Clear_Creek_Pred <- predict(Best_Fit_Model, n.ahead = 10*12)

ts.plot(Clear_Creek_MQ_ts, Clear_Creek_Pred) #lty = c(1,3))
```
